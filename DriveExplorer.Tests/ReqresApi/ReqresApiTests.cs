...
namespace DriveExplorer.ReqresApi {
	public class ReqresApiFixture : IDisposable {
		public HttpClient client { get; set; } = new HttpClient();

		public ReqresApiFixture(){
	...
	public class ReqresApiTests : IClassFixture<ReqresApiFixture> {
		private HttpClient client;

		public ReqresApiTests(ReqresApiFixture fixture) {
			client = fixture.client;
			Debug.WriteLine(client.GetHashCode());
		}
		...
		public async Task UpdateUser_IdenticalUserNameAsync() {
			...
			//When
			using var response = await client.PutAsync(url, requestBody);
			var responseBody = await response.Content.ReadAsStringAsync();
			Console.WriteLine(response.StatusCode);
			Console.WriteLine(responseBody);
			var jObject = JObject.Parse(responseBody);
		...
		public async Task PatchUser_IdenticalUserNameAsync() {
			...
			var requestBody = new StringContent("{\"name\":\"" + name + "\",\"job\":\"leader\"}", Encoding.UTF8, ContentTypes.Application.json);
			//When
			using var response = await client.PatchAsync(url, requestBody);
			var responseBody = await response.Content.ReadAsStringAsync();
			Console.WriteLine(response.StatusCode);
			Console.WriteLine(responseBody);
			var jObject = JObject.Parse(responseBody);
		...
		public async Task RegisterUser_SuccessResponseAsync() {
			...
			//When
			using var response = await client.PostAsync(url, requestBody);
			var responseBody = await response.Content.ReadAsStringAsync();
			Console.WriteLine(response.StatusCode);
			Console.WriteLine(responseBody);
			//Then
